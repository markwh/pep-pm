...
paramdf <- resultList %>% 
  map(~data.frame(pname = names(.$params), value = .$params,
                  stringsAsFactors = FALSE)) %>% 
  bind_rows(.id = "listid") %>% 
  mutate(param = markstats::splitPiece(pname, "_", 1),
         listid = as.numeric(listid),
         locid = as.numeric(markstats::splitPiece(pname, "_", 2))) %>% 
  left_join(mutate(resultMatrix, listid = msg)) %>% 
  select(pname, value, param, locid, case, fl, mc, metric, method) %>% 
  spread(key = method, value = value) %>% 
  mutate(part_optim = ifelse(param == "logA0", stats, part_optim)) 

# TODO: make remaining paramdf_joined modifications to match earlier version
...
slidepardf <- paramdf_joined %>% 
  mutate(method = factor(method, levels = c("stats", "optim")),
         fl = plyr::mapvalues(fl, from = c("bam_man", "metroman", "omniscient"),
                              to = c("fl:BAM", "fl:Met", "fl:Omni")),
         mc = plyr::mapvalues(mc, from = c("bam", "metroman", "omniscient"),
                              to = c("mc:BAM", "mc:Met", "mc:Omni")),
         shortparam = markstats::splitPiece(paramname, split = "_", piece = 1),
         flparam = paste(fl, shortparam, sep = "_")) %>% 
  # summary()
