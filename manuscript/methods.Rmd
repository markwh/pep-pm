...
A discharge estimation algorithm is a function, $g$, of parameters, $\theta$, and data, $\mathbf{x}$. The values of this function constitute predictions of dischrge, $\hat{Q}$ at a particular time, $t$, and location, $s$. That is:

$$
\hat{Q}_{st} = g(\theta, \mathbf{x})
$$

In order to assess the performance of a given discharge algorithm, we rely on benchmark datasets that contain both $\mathbf{x}$--as would be available to SWOT--as well as values of true discharge, $Q_{st}$--which would not.
...
The best-achievable performance of a discharge algorithm is defined in relation to a performance metric, which summarizes the mismatch between $\hat{Q}$ and true discharge, $Q$, over a number of of locations and times. There are at least two interpretations of this. 
